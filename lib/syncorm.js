...
/*					U.each(rows, function(row) {
	...
	if (typeof params.maxListeners === 'number') {
		this.$events.setMaxListeners(params.maxListeners);
	} else {
	...
	this._refreshDatabase = function(opts, cb) {
		...
		self.$driver.getAll('dbops',
			...
			function(err, rowsss) {
				...
				const rowsWithoutDuplicated = rowsss.reduce((acc, row) => {
					const rowMainKey = row.table + '$' + row.op + '$' + row.key + '$' + row.datetime;
					if (!processedInstances.includes(rowMainKey)) {
						acc.push(row);
						processedInstances.push(rowMainKey);
					}
					return acc;
				}, []);

				rowsWithoutDuplicated.sort(function(a,b) {
					return a.id - b.id;
				});

				async.eachSeries(rowsWithoutDuplicated, function(row, cb2) {
...
//								console.log("row: " + JSON.stringify(row) +  "  result: " + JSON.stringify(rows2));
					...
					U.each(rowsWithoutDuplicated, function(row) {
						if (row.clsName) {
							var sch = self[row.clsName].prototype.$schema;
							if (row.op === "INSERT") {
